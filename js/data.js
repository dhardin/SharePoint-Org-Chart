...
app.data = (function() {
        ...
        $.ajax({
            ...
            complete: function(xData, status) {
                var error = $(xData.responseText).find('ErrorText').text(),
                    results = {
                        data: $(xData.responseText).find('z\\:row'),
                        error: error
                    };
    ...
    _buildPayload = function(arr, index, obj_map, method, payload, callback) {
        ...
        if (!(arr instanceof Array) || !(obj_map instanceof Object) || !(method_map.hasOwnProperty(method.toLowerCase()))) {
            return false;
        }
