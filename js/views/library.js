...
app.LibraryView = Backbone.View.extend({
            ...
            buildTable: function(parent, collection) {
                ...
                if (children.length > 0 && collection.length > 1) {
                    ...
                    for (i in children) {
                        ...
                        $td = $('<td colspan="2">');
                        (function(td, p, that, col, index) {
                            setTimeout(function() {
                                td.append(that.buildTable(p.get('children')[index], col));
                            }, 1000);

                        })($td, parent, this, collection, i);
                        $tr.append($td);
                    }
            ...
            buildCondensedTable: function(parent, collection) {
                var children = parent.get('children'),
                    that = this,
                    nodeColSpan, downLineTable, linesCols, i, $tr, $td,
                    $table = $('<table cellpadding="0" cellspacing="0" border="0">');

                nodeColspan = children.length > 0 ? 2 * (this.getChildrenWithChildren(parent, children).length + (this.getChildrenWithChildren(parent, children).length < parent.get('children').length ? 1 : 0)) : 2;
                ...
                if (children.length == 0) {
                    if (!this.parent_child_collection_cache[parent.get('parent')]) {
                        //create new node item collection
                        childrenCollection = new app.Library();
                        //add model to node item collection
                        this.parent_child_collection_cache[parent.get('parent')] = childrenCollection;
                        childrenCollection.add(parent);
                        ...
                        this.parent_child_collection_cache[parent.get('parent')].add(parent);
                        return false;
                    }
                } else {
                ...
                $table.append($tr);
                if (children.length > 0 && collection.length > 1) {
                    $table.append(this.buildBranches(this.getChildrenWithChildren(parent, children).length + (this.getChildrenWithChildren(parent, children).length < parent.get('children').length ? 1 : 0)));
                    $tr = $("<tr>");
                    for (i in children) {
                        var child = children[i];
                        if (collection.indexOf(child) == -1) {
